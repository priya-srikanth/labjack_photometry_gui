...
Analog lick on `AIN6` is disabled by default because it can bleed into detector channels on this rig. Use the lick detector board output on `FIO4` for photometry alignment. If you also need position strobe in the LabJack file, wire the Teensy strobe output on Teensy pin `20` to CB37-accessible `MIO0` and enable that row in the GUI.
